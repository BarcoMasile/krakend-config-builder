...
$parser = OptionParser.new do |opts|
  ...
  opts.on("-x", "--extra CONFIG-PLUGINS", Array, "Extra config plugin list") do |extraPlugins|
    $args[:extra_config] = extraPlugins
  end

  opts.on("-e", "--endpoints ENDPOINT-PLUGINS", Array, "Endpoint Plugin list") do |endpoints|
  ...
  opts.on("--output-encoding ENCODING", String, "Output encoding") do |output_encoding|
    $args[:output_encoding] = output_encoding
  end

  opts.on("-o", "--output-file FILENAME", String, "Output file") do |file_name|
